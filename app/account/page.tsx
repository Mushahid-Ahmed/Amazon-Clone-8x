...
export default function AccountPage() {
  const { user, authUser, orders, cartItemCount, setPrime, addAddress, removeAddress, setDefaultAddress, hydrated } = useStore();
  const effectiveUser = authUser ?? user;
  const address = effectiveUser.addresses.find((item) => item.isDefault) ?? effectiveUser.addresses[0];
  ...
  async function submitAddress(event: FormEvent<HTMLFormElement>) {
    ...
    setFormError("");
    try {
      await addAddress({ ...form, line2: form.line2 || undefined, isDefault: effectiveUser.addresses.length === 0 });
      setForm(EMPTY_FORM);
      setAdding(false);
    } catch (error) {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6">
      ...
      <p className="mt-1 text-slate-600">Welcome back, {effectiveUser.name}.</p>

      {!authUser && hydrated && (
        <p className="mt-4 rounded-lg border border-sky-300 bg-sky-50 px-4 py-3 text-sm text-sky-800">
          You&apos;re viewing the demo profile. <Link href="/auth?redirect=/account" className="font-semibold text-amazon-link hover:underline">Sign in</Link> to manage your real account, addresses, and orders.
        </p>
      )}
      ...
      <div className="mt-7 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
        <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <p className="mt-3 font-semibold">{effectiveUser.name}</p>
          <p className="text-sm text-slate-600">{effectiveUser.email}</p>
          <button type="button" onClick={() => setPrime(!effectiveUser.isPrime)} className="mt-5 flex items-center gap-2 text-sm font-semibold text-amazon-link">
            <span className={`rounded-full px-2 py-0.5 text-xs font-bold ${effectiveUser.isPrime ? "bg-sky-100 text-sky-800" : "bg-slate-100 text-slate-600"}`}>prime</span>
            {effectiveUser.isPrime ? "Prime is active · Turn off" : "Try Prime · Turn on"}
          </button>
          {authUser && (
      ...
      <section className="mt-6 rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
        ...
        {effectiveUser.addresses.length === 0 ? (
          ...
          <ul className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {effectiveUser.addresses.map((item) => (
