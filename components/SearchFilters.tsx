...
export default function SearchFilters({ products }: { products: Product[] }) {
  ...
  const content = <div className="space-y-6 text-sm">
    ...
    <label className="flex items-center gap-2 font-medium"><input type="checkbox" checked={params.has("prime")} onChange={(event) => update({ prime: event.target.checked ? "1" : null })} /> Prime eligible</label>
    <label className="flex items-center gap-2 font-medium"><input type="checkbox" checked={params.has("deals")} onChange={(event) => update({ deals: event.target.checked ? "1" : null })} /> Today&apos;s deals</label>
  </div>;
