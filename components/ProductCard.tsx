  ...
  return (
    ...
    >
      <div className="relative mb-3 aspect-square overflow-hidden rounded-md bg-slate-100">
        <Image src={product.images[0]} alt={`${product.title} product image`} fill sizes="(max-width: 640px) 45vw, 220px" className="object-cover transition-transform duration-300 group-hover:scale-105 group-hover:opacity-0" />
        <Image src={product.images[1]} alt={`${product.title} alternate product image`} fill sizes="(max-width: 640px) 45vw, 220px" className="object-cover opacity-0 transition-all duration-300 group-hover:scale-105 group-hover:opacity-100" />
        {product.discount && (
