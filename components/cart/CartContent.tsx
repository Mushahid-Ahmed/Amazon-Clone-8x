  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6">
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_320px]">
        ...
        <aside className="h-fit rounded-lg border border-slate-300 bg-white p-5 lg:sticky lg:top-4"><p className={`mb-4 flex items-center gap-2 text-sm ${qualifies ? "text-emerald-700" : "text-slate-600"}`}>{qualifies && <Check size={17} />}{qualifies ? "Your order qualifies for FREE Shipping" : `Add $${Math.max(0, 35 - cartSubtotal).toFixed(2)} to qualify for FREE Shipping`}</p><div className="flex justify-between text-lg"><span>Subtotal ({cartItemCount} items):</span><strong>${cartSubtotal.toFixed(2)}</strong></div><button type="button" disabled={cart.length === 0} className="mt-5 w-full rounded-full bg-amazon-yellow px-4 py-3 font-medium hover:bg-amber-400 disabled:cursor-not-allowed disabled:opacity-50">Proceed to checkout</button><p className="mt-4 flex gap-2 border-t border-slate-200 pt-4 text-xs text-slate-600"><ShieldCheck size={16} className="shrink-0" /> Secure payments and easy returns.</p></aside>
      </div>
    </div>
  );
}
