...
import { Check, Minus, Plus, ShieldCheck, Trash2, ShoppingCart } from "lucide-react";
        ...
        <div className="space-y-6">
          {cart.length === 0 ? (
            <section className="rounded-lg border border-slate-300 bg-white p-10 text-center">
              <ShoppingCart size={52} className="mx-auto text-slate-400" aria-hidden="true" /><h2 className="mt-4 text-xl font-bold">Your Amazon Clone Cart is empty</h2><p className="mt-2 text-slate-600">Add products from the store to see them here.</p><Link href="/deals" className="mt-5 inline-block rounded-full bg-amazon-yellow px-6 py-2 font-medium hover:bg-amber-400">Shop today&apos;s deals</Link>
            </section>
          ) : <section className="rounded-lg border border-slate-300 bg-white p-5"><h2 className="mb-4 text-xl font-bold">Cart ({cartItemCount} items)</h2><ul className="divide-y divide-slate-200">{cart.map(({ product, quantity }) => <li key={product.id} className="flex min-w-0 gap-3 py-5 first:pt-0 sm:gap-4"><div className="relative h-24 w-24 shrink-0 sm:h-36 sm:w-36"><Image src={product.images[0]} alt={product.title} fill sizes="(max-width: 640px) 96px, 144px" className="rounded object-cover" /></div><div className="min-w-0 flex-1"><Link href={`/product/${product.id}`} className="line-clamp-2 font-medium text-amazon-link hover:underline">{product.title}</Link><p className="mt-1 text-sm text-emerald-700">In Stock · FREE delivery</p><p className="mt-2 text-lg font-bold">${product.price.toFixed(2)}</p><div className="mt-3 flex flex-wrap items-center gap-3 text-sm"><div className="flex items-center rounded border border-slate-300"><button type="button" disabled={quantity <= 1} onClick={() => updateCartQuantity(product.id, quantity - 1)} className="p-2 disabled:opacity-40" aria-label="Decrease quantity"><Minus size={15} /></button><span className="min-w-8 text-center">{quantity}</span><button type="button" disabled={quantity >= 10} onClick={() => updateCartQuantity(product.id, quantity + 1)} className="p-2 disabled:opacity-40" aria-label="Increase quantity"><Plus size={15} /></button></div><button type="button" onClick={() => removeFromCart(product.id)} className="text-amazon-link hover:underline"><Trash2 size={14} className="mr-1 inline" />Delete</button><button type="button" onClick={() => { saveForLater(product); removeFromCart(product.id); }} className="text-amazon-link hover:underline">Save for later</button></div></div><strong className="hidden shrink-0 text-right text-lg sm:block">${(product.price * quantity).toFixed(2)}</strong></li>)}</ul></section>}
